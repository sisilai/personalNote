define(function (require, exports, module) {
    ...
    function initController($scope, $location) {
                    ...
                    function _markRecordItem() {
                        ...
                        _.each(recordCard, function (card) {
                            ...
                            _.each(itemIds, function (itemId) {
                                if (!$scope.isRecordContainsService(card, itemId)) {
                                    return;
                                }
                                ...
                                if (!_.isEmpty(copyItem)) {
                                    ...
                                    copyItem.remainingTimes = card.balance;
        ...
        $scope.removeItemPayInfo = function (product) {
            if (_.isEmpty(product) || _.isEmpty(product.payCard)) {
                return;
            }

            var payCard = product.payCard;
            var cardPayInfo = product.cardPayInfo;

            // 年卡支付方式移除，可能导致其他项目被该年卡支付的有效次数变化
            if (cardPayInfo.payType === 'quarterCard') {
                _adjustOtherQuarterCardPayActiveTimes();
            }

            delete product.payCard;
            delete product.cardPayInfo;

            $("#m-pos-member-card-select .y_dialog_header .y_step_status>li:nth-child(3)").show();

            function _adjustOtherQuarterCardPayActiveTimes() {
                var currentItemActiveTimes = cardPayInfo.activeTimes;
                ...
                var payBySameCardAndActiveTimesShortItem = _.filter($scope.buyProductRecords, function (item) {
                    return !_.isEmpty(item.payCard) && item.payCard.id === payCard.id && item.cardPayInfo.activeTimes !== item.cardPayInfo.cardPayTimes;
                });

                _.each(payBySameCardAndActiveTimesShortItem, function (item) {
                    var itemCardPayInfo = item.cardPayInfo;

                    var shortTimes = itemCardPayInfo.cardPayTimes - itemCardPayInfo.activeTimes;
                    ...
                    if (currentItemActiveTimes >= shortTimes) {
                        itemCardPayInfo.activeTimes += shortTimes;

                    }
                    else if (currentItemActiveTimes < shortTimes) {
                        itemCardPayInfo.activeTimes += currentItemActiveTimes;
                    }

                    itemCardPayInfo.times2Money = itemCardPayInfo.activeTimes * item.payCard.timesAvgPrice;
